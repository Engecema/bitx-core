// Copyright (c) 2025-present The BitX Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries bitx-node, bitx-gui.
  -M, --monolithic       Run monolithic binaries bitxd, bitx-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'bitx-qt [ARGS]' or 'bitx-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'bitxd [ARGS]' or 'bitx-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'bitx-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'bitx-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'bitx-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_bitx [ARGS]'.
  chainstate [ARGS] Run bitx kernel chainstate util, equivalent to running 'bitx-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_bitx [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_bitx-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "bitx-gui" : "bitx-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "bitx-node" : "bitxd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("bitx-cli");
            // Since "bitx rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("bitx-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("bitx-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_bitx");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("bitx-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_bitx");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_bitx-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("bitx-util");
        } else {
...
//! Execute the specified bitxd, bitx-qt or other command line in `args`
...
//!             be a relative path to a bitxd, bitx-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/bitx ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/pull/31375#discussion_r1861814807)
