...
namespace init {
namespace {
const char* EXE_NAME = "bitx-node";

class BitXNodeInit : public interfaces::Init
{
public:
    BitXNodeInit(node::NodeContext& node, const char* arg0)
        : m_node(node),
...
{
    auto init = std::make_unique<init::BitXNodeInit>(node, argc > 0 ? argv[0] : "");
    // Check if bitx-node is being invoked as an IPC server. If so, then
