...
namespace init {
namespace {
const char* EXE_NAME = "bitx-gui";

class BitXGuiInit : public interfaces::Init
{
public:
    BitXGuiInit(const char* arg0) : m_ipc(interfaces::MakeIpc(EXE_NAME, arg0, *this))
    {
        InitContext(m_node);
        m_node.init = this;
    }
    ...
    // bitx-gui accepts -ipcbind option even though it does not use it
    // directly. It just returns true here to accept the option because
    // bitx-node accepts the option, and bitx-gui accepts all bitx-node
...
{
    return std::make_unique<init::BitXGuiInit>(argc > 0 ? argv[0] : "");
}
} // namespace interfaces
