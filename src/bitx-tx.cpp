...
static void SetupBitXTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " bitx-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The bitx-tx tool is used for creating and modifying bitx transactions.\n\n"
                "bitx-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded bitx transaction, or with \"-create [commands]\" to create a hex-encoded bitx transaction.\n"
                "\n"
                "Usage: bitx-tx [options] <hex-tx> [commands]\n"
                "or:    bitx-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitx transaction
