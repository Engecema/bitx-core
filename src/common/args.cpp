...
const char * const BITX_CONF_FILENAME = "bitx.conf";
const char * const BITX_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitx-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", BITX_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\BitX
    //   new: C:\Users\Username\AppData\Local\BitX
    // macOS: ~/Library/Application Support/BitX
    // Unix-like: ~/.bitx
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "BitX";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "BitX";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/BitX";
#else
    // Unix-like
    return pathRet / ".bitx";
#endif
