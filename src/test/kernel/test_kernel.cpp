...
#include <kernel/bitxkernel.h>
#include <kernel/bitxkernel_wrapper.h>
#include <util/byte_units.h>
#include <util/fs.h>

// Boost.Test's SIGSTKSZ alternate stack can be smaller than Linux requires on musl.
#define BOOST_TEST_DISABLE_ALT_STACK
#define BOOST_TEST_MODULE BitX Kernel Test Suite
...
{
    Logger logger{std::make_unique<TestLog>()};
    auto test_directory{TestDirectory{"chainman_test_bitx_kernel"}};
...
{
    auto test_directory{TestDirectory{"mainnet_test_bitx_kernel"}};
...
{
    auto test_directory{TestDirectory{"block_tree_entry_test_bitx_kernel"}};
    auto notifications{std::make_shared<TestKernelNotifications>()};
...
{
    auto in_memory_test_directory{TestDirectory{"in-memory_test_bitx_kernel"}};
...
{
    auto test_directory{TestDirectory{"regtest_test_bitx_kernel"}};
...
BOOST_AUTO_TEST_CASE(btck_set_mock_time_tests)
{
    ...
    auto test_directory{TestDirectory{"set_mock_time_test_bitx_kernel"}};
    auto notifications{std::make_shared<TestKernelNotifications>()};
    auto context{create_context(notifications, ChainType::REGTEST)};
    auto chainman{create_chainman(
        test_directory, /*reindex=*/false, /*wipe_chainstate=*/false,
