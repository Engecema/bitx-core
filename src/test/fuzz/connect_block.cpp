                                   ...
                                   int target_height = 0)
{
    ...
    if (coinbase) {
        ...
        LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 10) {
            ...
            if (input_index < g_spend_candidate_txins.size()) {
                ...
                txin = g_spend_candidate_txins[input_index];
            } else {
                // Pick it in the additional_txins set.
                Assert((input_index - g_spend_candidate_txins.size()) < additional_txins.size());
                txin = additional_txins[input_index - g_spend_candidate_txins.size()];
            }

            // Enable the fuzzer to mutate every CTxIn field after it is taken
            // from the spend candidates.
            if (fuzzed_data_provider.ConsumeBool()) {
                txin.nSequence = ConsumeSequence(fuzzed_data_provider);
            }
            if (fuzzed_data_provider.ConsumeBool()) {
                txin.prevout.n = fuzzed_data_provider.ConsumeIntegral<uint32_t>();
            }
            if (fuzzed_data_provider.ConsumeBool()) {
                txin.prevout.hash = Txid::FromUint256(ConsumeUInt256(fuzzed_data_provider));
            }
            if (fuzzed_data_provider.ConsumeBool()) {
    ...
    LIMITED_WHILE(tx.vout.empty() || fuzzed_data_provider.ConsumeBool(), 10) {
        ...
        CallOneOf(
            ...
            [&] {
                // P2WSH
                txout.scriptPubKey = P2WSH_OP_TRUE;
            },
            [&] {
                // P2SH
                txout.scriptPubKey = P2SH_OP_TRUE;
            },
            [&] {
                // Taproot
                txout.scriptPubKey = TAPROOT_OP_TRUE;
            },
            [&] {
                // Empty script
            ...
            [&] {
                ...
                txout.scriptPubKey = ConsumeScript(fuzzed_data_provider);
            });
    }
                    ...
                    std::vector<CTxIn>& additional_txins)
{
    ...
    // Read up to 5 transactions from the input.
