...
RPCMethod removeprunedfunds()
{
    return RPCMethod{
        "removeprunedfunds",
        "(DEPRECATED) This feature will be removed in the next major release. Start bitxd with the `-deprecatedrpc=removeprunedfunds` option in order to use this.\n"
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!pwallet->chain().rpcEnableDeprecated("removeprunedfunds")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "DEPRECATION WARNING: This feature will be removed in the next major release. Start bitxd with the `-deprecatedrpc=removeprunedfunds` option in order to use this.");
    }
