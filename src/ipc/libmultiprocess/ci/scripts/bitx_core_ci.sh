...
run_ipc_unit_tests() {
  ...
  for _ in $(seq 1 "${runs}"); do
    build/bin/test_bitx --run_test=ipc_tests,miner_tests --catch_system_error=no --log_level=nothing --report_level=no
  done
}
