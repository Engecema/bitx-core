...
thread_local ThreadContext g_thread_context; // NOLINT(bitx-nontrivial-threadlocal)
...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking bitx code so they have to run
    ...
    // functions run, so client objects are fully disconnected before bitx
    // code in the destructors are run. This way if the bitx code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In bitx core, running these callbacks in the right order is
