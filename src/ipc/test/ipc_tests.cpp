...
void IpcSocketTest(const fs::path& datadir)
{
    ...
    BOOST_CHECK_THROW(process->bind(datadir, "test_bitx", invalid_bind), std::invalid_argument);
    BOOST_CHECK_THROW(process->connect(datadir, "test_bitx", invalid_bind), std::invalid_argument);

    auto bind_and_listen{[&](const std::string& bind_address) {
        std::string address{bind_address};
        mp::SocketId serve_fd = process->bind(datadir, "test_bitx", address);
    ...
    auto connect_and_test{[&](const std::string& connect_address) {
        ...
        mp::SocketId connect_fd{process->connect(datadir, "test_bitx", address)};
    ...
    //   Address 'unix' path '"/tmp/test_common_BitX Core/ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff/test_bitx.sock"' exceeded maximum socket path length
    std::vector<std::string> addresses{
        strprintf("unix:%s", TempPath("bitx_sock0_XXXXXX")),
        strprintf("unix:%s", TempPath("bitx_sock1_XXXXXX")),
    };
...
BOOST_AUTO_TEST_CASE(parse_address_test)
{
    ...
    auto check_address{[&](std::string address, std::string expect_address, std::string expect_error) {
        if (expect_error.empty()) {
            BOOST_CHECK_EXCEPTION(process->connect(datadir, "test_bitx", address), std::system_error, check_notexist);
        } else {
            BOOST_CHECK_EXCEPTION(process->connect(datadir, "test_bitx", address), std::invalid_argument, HasReason(expect_error));
        }
        BOOST_CHECK_EQUAL(address, expect_address);
    }};
    std::string prefix{fs::PathToString(datadir / "")};
    check_address("unix", "unix:" + prefix + "test_bitx.sock", "");
    check_address("unix:", "unix:" + prefix + "test_bitx.sock", "");
