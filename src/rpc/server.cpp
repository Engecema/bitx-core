...
void ApplyTypeStrOverride(UniValue& schema, const RPCArg& arg)
{
    ...
    if (number_or_string.contains(type_label)) {
        ...
        schema.pushKV("x-bitx-type-override", type_label);
    }
}
...
void ApplyArgFallback(UniValue& schema, const RPCArg& arg)
{
    std::visit(util::Overloaded{
                   ...
                   [&](const RPCArg::DefaultHint& hint) { schema.pushKV("x-bitx-default-hint", hint); },
...
UniValue OpenRPCArgSchema(const RPCArg& arg, bool include_hidden, bool in_skip_type_check)
{
    ...
    case RPCArg::Type::OBJ_NAMED_PARAMS: {
        ...
        for (const auto& inner : arg.m_inner) {
            ...
            if (inner.m_opts.placeholder) prop.pushKV("x-bitx-placeholder", true);
            if (inner.m_opts.also_positional) prop.pushKV("x-bitx-also-positional", true);
...
UniValue OpenRPCResultSchema(const RPCResult& result)
{
    ...
    case RPCResult::Type::STR_AMOUNT:
        return MakeObject({{"type", "number"}, {"x-bitx-unit", "amount"}});
    ...
    case RPCResult::Type::NUM_TIME: {
        ...
        schema.pushKV("x-bitx-unit", "unix-time");
        return schema;
    }
            ...
            {RPCResult::Type::OBJ, "info", "Metadata about this JSON-RPC interface.",
                {
                    ...
                    {RPCResult::Type::STR, "version", "BitX Core version string."},
                            ...
                            {{RPCResult::Type::OBJ, "", "A parameter.",
                                {
                                    ...
                                    {RPCResult::Type::ARR, "x-bitx-aliases", /*optional=*/true, "Alternative parameter names.",
                                        {{RPCResult::Type::STR, "", "An alias."}}},
                                    {RPCResult::Type::BOOL, "x-bitx-placeholder", /*optional=*/true, "Whether the parameter is retained only for compatibility."},
                                    {RPCResult::Type::BOOL, "x-bitx-also-positional", /*optional=*/true, "Whether the parameter can also be passed positionally."},
                                }}}},
                        {RPCResult::Type::OBJ, "result", "Method result.",
                            {
                                ...
                                {RPCResult::Type::ANY, "schema", "JSON Schema for the result. Numeric schemas may include "
                                    "\"x-bitx-unit\" property: \"amount\" which denotes a BitX amount in BTC."},
                            }},
                        {RPCResult::Type::STR, "x-bitx-category", "RPC category."},
                    }}}},
        },
    ...
    for (const auto& method_name : method_names) {
        ...
        for (const auto& arg : helpman.GetArgs()) {
            ...
            if (names.size() > 1) {
                ...
                param.pushKV("x-bitx-aliases", std::move(aliases));
            }
            if (arg.m_opts.placeholder) param.pushKV("x-bitx-placeholder", true);
            if (arg.m_opts.also_positional) param.pushKV("x-bitx-also-positional", true);
        ...
        method.pushKV("x-bitx-category", cmd->category);
