...
//! Specify whether parameter should be parsed by bitx-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make bitx-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `bitx-cli` attempt to parse the
 ...
 * `bitx-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make bitx-cli treat them as positional parameters when `-named` is used.
 * This prevents `bitx-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so bitx-cli
 ...
 *
 * For example, the command `bitx-cli -named createwallet "my=wallet"`,
