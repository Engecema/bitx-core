...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<BitXUnit>("BitXUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitx_de.qm (shortcut "de" needs to be defined in bitx.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitx_de_DE.qm (shortcut "de_DE" needs to be defined in bitx.qrc)
...
static const char* qt_argv = "bitx-qt";

BitXApplication::BitXApplication()
...
void BitXApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BitXApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitXGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BitXApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BitXApplication::createOptionsModel(bool resetSettings)
{
...
void BitXApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BitXGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BitXGUI::quitRequested, this, &BitXApplication::requestShutdown);
...
void BitXApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BitXApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BitXApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BitXApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BitXApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BitXApplication::handleRunawayException);
    connect(this, &BitXApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BitXApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BitXApplication::parameterSetup()
{
...
void BitXApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BitXApplication::requestInitialize()
{
...
void BitXApplication::requestShutdown()
{
...
void BitXApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    // bitx: URIs or payment requests:
    if (paymentServer) {
        connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BitXGUI::handlePaymentRequest);
        connect(window, &BitXGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BitXApplication::handleRunawayException(const QString &message)
{
...
void BitXApplication::handleNonFatalException(const QString& message)
{
...
WId BitXApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BitXApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitXGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bitx);
    Q_INIT_RESOURCE(bitx_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(BITX_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bitx-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see bitx-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse bitx.conf, determine network, switch to network specific
    ...
    // bitx: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
