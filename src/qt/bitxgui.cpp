...
#include <bitx-build-config.h> // IWYU pragma: keep

#include <qt/bitxgui.h>

#include <qt/bitxunits.h>
 ...
 *
 * Ref: https://github.com/pull/1026
 */
static constexpr int64_t MAX_BLOCK_TIME_GAP = 90 * 60;

const std::string BitXGUI::DEFAULT_UIPLATFORM =
...
BitXGUI::BitXGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &BitXGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BitXGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BitXGUI::showModalOverlay);
...
void BitXGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BitX address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bitx: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &BitXGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &BitXGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BitXGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BitX addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BitX addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed BitX Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed BitX Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a bitx: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BitX command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &BitXGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &BitXGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &BitXGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &BitXGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &BitXGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &BitXGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &BitXGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BitXGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &BitXGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &BitXGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &BitXGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &BitXGUI::enableHistoryAction);
    ...
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &BitXGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &BitXGUI::showDebugWindow);
}

void BitXGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void BitXGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &BitXGUI::setCurrentWalletBySelectorIndex);
...
void BitXGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &BitXGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &BitXGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &BitXGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &BitXGUI::showProgress);
...
#ifdef ENABLE_WALLET
void BitXGUI::enableHistoryAction(bool privacy)
{
...
void BitXGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &BitXGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &BitXGUI::removeWallet);
...
WalletController* BitXGUI::getWalletController()
{
    return m_wallet_controller;
}

void BitXGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &BitXGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &BitXGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &BitXGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &BitXGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &BitXGUI::incomingTransaction);
    connect(this, &BitXGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void BitXGUI::removeWallet(WalletModel* walletModel)
{
...
void BitXGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void BitXGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void BitXGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BitXGUI::setWalletActionsEnabled(bool enabled)
{
...
void BitXGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BitXGUI::toggleHidden);
...
void BitXGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void BitXGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BitXGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void BitXGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void BitXGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void BitXGUI::openClicked()
{
...
void BitXGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BitXGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BitXGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BitXGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BitXGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BitXGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void BitXGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void BitXGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to BitX network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BitXGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BitXGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BitXGUI::updateHeadersSyncProgressLabel()
{
...
void BitXGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void BitXGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &BitXGUI::quitRequested);
...
void BitXGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void BitXGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &BitXGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void BitXGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void BitXGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &BitXGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &BitXGUI::show);
                e->ignore();
            }
        }
    }
...
void BitXGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BitXGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BitXGUI::incomingTransaction(const QString& date, BitXUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BitXUnits::formatWithUnit(unit, amount, true));
...
void BitXGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BitXGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BitXGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BitXGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BitXGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void BitXGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BitXGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void BitXGUI::updateWindowTitle()
{
...
void BitXGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BitXGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BitXGUI::detectShutdown()
{
...
void BitXGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void BitXGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

[[nodiscard]] static bool ThreadSafeMessageBox(BitXGUI* gui, const bilingual_str& message, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = BitXGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void BitXGUI::subscribeToCoreSignals()
{
...
void BitXGUI::unsubscribeFromCoreSignals()
{
...
bool BitXGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<BitXUnit> units = BitXUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BitXUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, BitXUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const BitXUnit u : BitXUnits::availableUnits()) {
        menu->addAction(BitXUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(BitXUnit newUnits)
{
    setText(BitXUnits::longName(newUnits));
}
