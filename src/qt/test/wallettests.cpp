                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<BitXAmountField*>("payAmount")->setValue(amount);
    Txid txid;
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = BitXUnits::formatWithUnit(unit, expected_balance, false, BitXUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     build/bin/test_bitx-qt  # Linux
//     QT_QPA_PLATFORM=windows build/bin/test_bitx-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   build/bin/test_bitx-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    BitXAmountField* amountInput = receiveCoinsDialog.findChild<BitXAmountField*>("reqAmount");
