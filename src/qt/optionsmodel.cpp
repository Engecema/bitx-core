...
/** Call node.updateRwSetting() with BitX 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because BitX 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bitx 22.x will not try to read these.
...
//! Convert enabled/size values to bitx -prune setting.
...
//! Get pruning enabled value to show in GUI from bitx -prune setting.
...
//! Get pruning size value to show in GUI from bitx -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBitXUnit")) {
        settings.setValue("DisplayBitXUnit", QVariant::fromValue(BitXUnit::BTC));
    }
    QVariant unit = settings.value("DisplayBitXUnit");
    if (unit.canConvert<BitXUnit>()) {
        m_display_bitx_unit = unit.value<BitXUnit>();
    } else {
        m_display_bitx_unit = BitXUnit::BTC;
        settings.setValue("DisplayBitXUnit", QVariant::fromValue(m_display_bitx_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bitx.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bitx_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BitXUnit>() == m_display_bitx_unit) return;
    m_display_bitx_unit = new_unit.value<BitXUnit>();
    QSettings settings;
    settings.setValue("DisplayBitXUnit", QVariant::fromValue(m_display_bitx_unit));
    Q_EMIT displayUnitChanged(m_display_bitx_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/pull/8273
    ...
    // (httpshttps://github.com/bitx-core/gui/issues/567).
