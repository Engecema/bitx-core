# CJDNS support in BitX Core

It is possible to run BitX Core over CJDNS, an encrypted IPv6 network that
...
done outside of BitX Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BitX network and individual nodes.
...
## Run BitX Core with CJDNS

Once you are connected to the CJDNS network, the following BitX Core
...
When enabled, this option tells BitX Core that it is running in an
...
IPv6 local network. This helps BitX Core perform better address management:
...
CJDNS support was added to BitX Core in version 23.0 and there may be fewer
...
[Sybil attacks](https://en.bitx.it/wiki/Weaknesses#Sybil_attack).
You can use `bitx-cli -addrinfo` to see the
...
## CJDNS-related information in BitX Core

There are several ways to see your CJDNS address in BitX Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitx-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitx-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bitx-cli help getnodeaddresses` for details.
