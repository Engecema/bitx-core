# Support for signing transactions outside of BitX Core

BitX Core can be launched with `-signer=<cmd>` where `<cmd>` is an external tool which can sign transactions and perform other functions. For example, it can be used to communicate with a hardware wallet.

Interaction with external signer uses [Partially Signed BitX Transaction (PSBT)](psbt.md).

## Example usage

The following example is based on the [HWI](https://github.com/bitx-core/HWI) tool. Version 2.0 or newer is required. Although this tool is hosted under the BitX Core GitHub organization and maintained by BitX Core developers, it should be used with caution. It is considered experimental and has far less review than BitX Core itself. Be particularly careful when running tools such as these on a computer with private keys on it.

When using a hardware wallet, consult the manufacturer website for (alternative) software they recommend. As long as their software conforms to the standard below, it should be able to work with BitX Core.

Start BitX Core:

```sh
bitxd -signer=../HWI/hwi.py
```

`bitx node` can also be substituted for `bitxd`.

### Device setup

Follow the hardware manufacturers instructions for the initial device setup, as well as their instructions for creating a backup. Alternatively, for some devices, you can use the `setup`, `restore` and `backup` commands provided by [HWI](https://github.com/bitx-core/HWI).
...
```sh
bitx rpc createwallet wallet_name="hww2" disable_private_keys=true descriptors=true external_signer=true
```

Creation of the external wallet can be confirmed with `getwalletinfo`, which will report `"external_signer": true`. These commands can also be executed using `bitx-qt` Debug Console instead of using `bitx rpc` or `bitx-cli`.
...
```sh
bitx-cli -rpcwallet=<walletname> getnewaddress
bitx-cli -rpcwallet=<walletname> walletdisplayaddress <address>
```
...
Under the hood this uses a [PSBT (Partially Signed BitX Transaction)](psbt.md).

```sh
bitx rpc -rpcwallet=<walletname> send outputs='{"<address>": <amount>}'
```

This constructs a PSBT and prompts your external signer to sign (will fail if it's not connected). If successful, BitX Core finalizes and broadcasts the transaction.
...
In order to be compatible with BitX Core, any signer command should conform to the specification below. This specification is subject to change. Ideally a BIP should propose a standard so that other wallets can also make use of it.

Prerequisite knowledge:
* [Output Descriptors](descriptors.md)
* Partially Signed BitX Transaction ([PSBT](psbt.md))

### Flag `--chain <name>` (required except for `enumerate`)

With `<name>` one of `main`, `test`, `signet`, `regtest`, `testnet4`.

BitX Core passes this flag to commands that operate on a chain-specific signer, for example `getdescriptors`, `displayaddress` and `signtx`.
...
BitX Core currently uses this flag for `signtx`.
...
BitX Core passes this flag to commands that operate on a specific external-signer wallet, for example `getdescriptors`, `displayaddress` and `signtx`.
...
## How BitX Core uses the Signer API
...
For external-signer wallets, spending uses `send` or `sendall`, and fee-bumping uses `bumpfee`. BitX Core builds a PSBT, adds key origin information, checks whether any input key origin fingerprint matches the signer, calls `<cmd> --stdin --fingerprint 00000000 --chain <name>`, and sends `signtx <psbt>` over stdin. If signatures are sufficient, it finalizes the transaction and, for broadcasting RPCs, broadcasts it. If signing cannot complete, the call fails with an error. For manual fee-bumping, use `psbtbumpfee` to obtain a PSBT for signing.
