# User-space, Statically Defined Tracing (USDT) for BitX Core

BitX Core includes statically defined tracepoints to allow for more
...
**Note**: BitX Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to BitX Core
...
Multiple tools can list the available tracepoints in a `bitxd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in BitX Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/bin/bitxd
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/bin/bitxd
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/bin/bitxd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in BitX Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/bin/bitxd | grep NT_STAPSDT -A 4 -B 2
