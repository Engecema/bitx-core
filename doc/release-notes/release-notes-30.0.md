...
BitX Core version v30.0 is now available from:

  <https://bitxcore.org/bin/bitx-core-30.0/>
...
installer (on Windows) or just copy over `/Applications/BitX-Qt` (on macOS)
...
BitX Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. BitX
...
- A new `bitx` command line tool has been added to make features more discoverable
  and convenient to use. The `bitx` tool just calls other executables and does not
  implement any functionality on its own. Specifically `bitx node` is a synonym for
  `bitxd`, `bitx gui` is a synonym for `bitx-qt`, and `bitx rpc` is a synonym
  for `bitx-cli -named`. Other commands and options can be listed with `bitx help`.
  The new `bitx` command is an alternative to calling other commands directly, but it
...
- The new `bitx` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `bitx -m node -ipcbind=unix` it will
  ...
  (`bitx-node` instead of `bitxd`) and is currently required but will become optional
  in the future (with [#33229](https://github.com/bitx/bitx/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md](https://github.com/bitx/bitx/blob/master/doc/multiprocess.md)),
...
- The `test_bitx` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `bitx` command
  as `bitx test`. The `libexec/` directory also contains new `bitx-node` and
  `bitx-gui` binaries which support IPC features and are called through the
  `bitx` tool. In source builds only, `test_bitx-qt`, `bench_bitx`, and
  `bitx-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `bitx` command. See `bitx help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of BitX Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in BitX Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the bitx-wallet tool,
...
[Transifex](https://explore.transifex.com/bitx/bitx/).
