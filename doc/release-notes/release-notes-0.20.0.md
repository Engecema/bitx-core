...
BitX Core version 0.20.0 is now available from:

  <https://bitxcore.org/bin/bitx-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  BitX
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use BitX Core on
unsupported systems.

From BitX Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, BitX Core does not yet change appearance
...
#### Removal of BIP61 reject network messages from BitX Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since BitX Core version 0.18.0.
...
- Testing or debugging of implementations of the BitX P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of BitX Core. BitX Core logs debug messages
...
- OpenSSL is no longer used by BitX Core.  (#17265)

- BIP70 support has been fully removed from BitX Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bitx/bitx/issues/16599),
  [PR #16702](https://github.com/bitx/bitx/pull/16702), and the
  `bitxd help` for more information.  This option is experimental and
...
- All custom settings configured when BitX Core starts are now
...
- The "Start BitX Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitxcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple BitX Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed BitX Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitx-core/HWI) for
  signing.  Future versions of BitX Core should support a GUI option
...
- BitX Core's automatically-generated source code documentation is
  now available at https://doxygen.bitxcore.org.  (#17596)
...
- The `bitx-cli` utility used with the `-getinfo` parameter now
...
- The `bitx-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `bitxstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bitx-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBBITX_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_bitx` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBBITX_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for bitx-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bitx schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitx.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bitx` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bitx-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bitx-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about BitX Core files (hebasto)
...
- #17119 Fix broken bitx-cli examples (andrewtoth)
...
- #17503 Remove bitness from bitx-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.bitxcore.org (laanwj)
...
- #17648 Rename wallet-tool references to bitx-wallet (hel-o)
...
- #16115 On bitxd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bitx-cli -getinfo (malevolent)
