...
Currently, it is possible to create a multisig wallet using BitX Core only.
...
Before starting this tutorial, start the bitx node on the signet network.

```bash
./build/bin/bitx node -signet -daemon
```

This tutorial also uses the default PKH derivation path to get the xpubs and does not conform to [BIP 45](https://github.com/bitx/bips/blob/master/bip-0045.mediawiki) or [BIP 87](https://github.com/bitx/bips/blob/master/bip-0087.mediawiki).
...
for ((n=1;n<=3;n++))
do
 ./build/bin/bitx rpc -signet createwallet "participant_${n}"
done
```

Extract the xpub of each wallet. To do this, the `derivehdkey` RPC is used.

Note that previously at least two descriptors were usually used, one for external derivation paths and one for internal ones. Since https://github.com/bitx/bitx/pull/22838 this redundancy has been eliminated by a multipath descriptor with <code><0;1></code> at the [BIP-44](https://github.com/bitx/bips/blob/master/bip-0044.mediawiki#change) change level expanding to external and internal descriptors when imported.
...
for ((n=1;n<=3;n++))
do
 xpubs["xpub_${n}"]=$(./build/bin/bitx rpc -signet -rpcwallet="participant_${n}" derivehdkey "m/44h/1h/0h" | jq -r '.origin + .xpub')
done
```
...
As previously mentioned, this step extracts the `m/44'/1'/0'` account instead of the path defined in [BIP 45](https://github.com/bitx/bips/blob/master/bip-0045.mediawiki) or [BIP 87](https://github.com/bitx/bips/blob/master/bip-0087.mediawiki), because the wallet currently can't sign for a derivation path that's not used in one of its descriptors.

### 1.2 Define the Multisig Descriptor

Define the multisig descriptors.

All RPCs in BitX Core will include the checksum in their output.

```bash
desc="wsh(sortedmulti(2,${xpubs["xpub_1"]}/<0;1>/*,${xpubs["xpub_2"]}/<0;1>/*,${xpubs["xpub_3"]}/<0;1>/*))"

desc_sum=$(./build/bin/bitx rpc -signet getdescriptorinfo $desc | jq -r '.checksum')

multisig_desc="[{\"desc\": \"$desc#$desc_sum\", \"active\": true, \"timestamp\": \"now\"}]"
```

`desc` specifies the output type (`wsh`, in this case) and the xpubs involved. It also uses BIP 67 (`sortedmulti`), so the wallet can be recreated without worrying about the order of xpubs. Conceptually, descriptors describe a list of scriptPubKey (along with information for spending from it) [[source](https://github.com/bitx/bitx/issues/21199#issuecomment-780772418)].
...
Documentation for these and other parameters can be found by typing `./build/bin/bitx rpc -signet help importdescriptors`.
...
```bash
./build/bin/bitx rpc -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/bitx rpc -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/bitx rpc -signet -rpcwallet="multisig_wallet_01" listdescriptors
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/bitx rpc -signet loadwallet "participant_${n}"; done
./build/bin/bitx rpc -signet loadwallet "multisig_wallet_01"
```
...
```bash
receiving_address=$(./build/bin/bitx rpc -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./build/bin/bitx-cli -a $receiving_address
```
...
```bash
./build/bin/bitx rpc -signet -rpcwallet="multisig_wallet_01" getbalances
```

### 1.5 Create a PSBT

Unlike singlesig wallets, multisig wallets cannot create and sign transactions directly because they require the signatures of the co-signers. Instead they create a Partially Signed BitX Transaction (PSBT).

PSBT is a data format that allows wallets and other tools to exchange information about a BitX transaction and the signatures necessary to complete it. [[source](https://bitxops.org/en/topics/psbt/)]

The current PSBT version (v0) is defined in [BIP 174](https://github.com/bitx/bips/blob/master/bip-0174.mediawiki).

For simplicity, the destination address is taken from the `participant_1` wallet in the code above, but it can be any valid bitx address.
...
```bash
balance=$(./build/bin/bitx rpc -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/bitx rpc -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/bitx rpc -signet -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```

There is also the `createpsbt` RPC, which serves the same purpose, but it has no access to the wallet or to the UTXO set. It is functionally the same as `createrawtransaction` and just drops the raw transaction into an otherwise blank PSBT. [[source](https://bitxtalk.org/index.php?topic=5131043.msg50573609#msg50573609)] In most cases, `walletcreatefundedpsbt` solves the problem.
...
```bash
./build/bin/bitx rpc -signet decodepsbt $funded_psbt

./build/bin/bitx rpc -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/bitx rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/bitx rpc -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/bitx rpc -signet combinepsbt txs="[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/bitx rpc -signet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/bitx rpc -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/bitx rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/bitx rpc -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/bitx rpc -signet finalizepsbt $psbt_2 | jq -r '.hex')
