...
Some notes on how to build BitX Core in Unix.
...
memory available when compiling BitX Core. On systems with less, gcc can be
...
To build BitX Core without the wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
When the intention is to only run a P2P node, without a wallet, BitX Core can
