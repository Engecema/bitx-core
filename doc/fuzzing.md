# Fuzzing BitX Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing BitX Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/bitx/bitx
$ cd bitx/
...
## Overview of BitX Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/bitx_report.pdf) includes an external overview of the status of BitX Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.
...
It is possible to specify `bitxd` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`bitx-core/qa-assets`](https://github.com/bitx-core/qa-assets) repo.

To fuzz `process_message` using the [`bitx-core/qa-assets`](https://github.com/bitx-core/qa-assets) seed corpus:

```sh
$ git clone --depth=1 https://github.com/bitx-core/qa-assets
...
an instrumentalized libc++, then using it to build [BitX Core dependencies
from source](../depends/README.md) and finally the BitX Core fuzz binary
...
the qa-assets repo
(https://github.com/bitx-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`bitx-core/qa-assets`](https://github.com/bitx-core/qa-assets) repo.

Every single pull request submitted against the BitX Core repo is automatically tested against all inputs in the [`bitx-core/qa-assets`](https://github.com/bitx-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make BitX Core more robust.
...
# Fuzzing BitX Core using afl++

## Quickstart guide

To quickly get started fuzzing BitX Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing BitX Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing BitX Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
BitX Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/bitx-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://issues.oss-fuzz.com/issues?q=bitx-core%20status:open).

BitX Core follows its [security disclosure policy](https://bitxcore.org/en/security-advisories/),
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_bitx-core/latest).
