...
Below are some notes on how to build BitX Core for Windows.

The options known to work for building BitX Core on Windows are:
...
Note that for WSL the BitX Core source path MUST be somewhere in the default mount file system, for
example /usr/src/bitx, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bitx`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/bitx
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/bitx --strip
```
