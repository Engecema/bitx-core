# bash programmable completion for bitx(1) wrapper
...
_bitx() {
    ...
    local bitx subcmd offset

    # save and use original argument to invoke bitx for help
    ...
    case "$subcmd" in
        gui|node)
            _bitx_wrap bitxd "$offset"
            return 0
            ;;
        rpc)
            _bitx_wrap bitx-cli "$offset"
            return 0
            ;;
        tx)
            _bitx_wrap bitx-tx "$offset"
            return 0
            ;;
    esac
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                options=$($bitx help 2>&1 | awk '{ for(i=1;i<=NF;i++) if ($i~/^--/) { sub(/=.*/, "=",$i); print $i } }' )
            fi
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$($bitx help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
