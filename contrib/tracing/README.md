...
Tracing (USDT) support for BitX Core on Linux using. For more information on
USDT support in BitX Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `bitxd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `bitxd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[BitX Core USDT documentation] on how to list available tracepoints in your
`bitxd` binary.

[BitX Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof bitxd)
```
...
BitX P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof bitxd)
```
...
In a different terminal, starting BitX Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/bitxd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof bitxd)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof bitxd)
```
