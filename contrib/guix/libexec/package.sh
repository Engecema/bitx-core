...
(
    ...
    (
        ...
        # copy over the example bitx.conf file. if contrib/devtools/gen-bitx-conf.sh
        # has not been run before building, this file will be a stub
        cp "${DISTSRC}/share/examples/bitx.conf" "${DISTNAME}/"
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/bitx-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
        ...
        *darwin*)
            ...
            mv build/dist/bitx-macos-app.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
