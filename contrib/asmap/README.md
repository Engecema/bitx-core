...
be used in BitX Core with the `-asmap` option.
...
to binary before being used with BitX Core.
...
the `bitx-cli getnodeaddresses` command.
...
```
bitx-cli getnodeaddresses 0 > addrs.json
```
