...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitx-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitx-cli', help='bitx-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='BitX address to which the faucet should send')
...
parser.add_argument('bitx_cli_args', nargs='*', help='Arguments to pass on to bitx-cli (default: -signet)')

args = parser.parse_args()

if args.bitx_cli_args == []:
    args.bitx_cli_args = ['-signet']


def bitx_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitx_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitx_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitx_cli(['getnewaddress', 'faucet', 'bech32'])
