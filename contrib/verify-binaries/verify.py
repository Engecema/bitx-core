...
"""Script for verifying BitX Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from bitxcore.org and bitx.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/bitx-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://bitxcore.org"
HOST2 = "https://bitx.org"
VERSIONPREFIX = "bitx-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the BitX community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"bitx_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by bitxcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the bitx release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes bitx.org lags behind bitxcore.org.)')
    )
