...
class ToolBitXTest(BitXTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `bitx node -version` is
        # run on windows, python doesn't capture output from the child
        # `bitxd` and `bitx-node` process started with _wexecvp, and
        # stdout/stderr are always empty. See
        # https://github.com/bitx/bitx/pull/33229#issuecomment-3265524908
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through bitx wrapper command with specified arguments."""
        # Manually construct the `bitx node` command, similar to Binaries::node_argv()
        bitx_cmd = node.binaries.valgrind_cmd + [node.binaries.paths.bitx_bin]
        node.args = bitx_cmd + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure bitx node command invokes bitxd by default")
        self.test_args([], [], expect_exe="bitxd")

        self.log.info("Ensure bitx -M invokes bitxd")
        self.test_args(["-M"], [], expect_exe="bitxd")

        self.log.info("Ensure bitx -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure bitx -m invokes bitx-node")
            self.test_args(["-m"], [], expect_exe="bitx-node")

            self.log.info("Ensure bitx -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="bitx-node")

            self.log.info("Ensure bitx accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="bitx-node")

            self.log.info("Ensure bitx recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="bitx-node")
