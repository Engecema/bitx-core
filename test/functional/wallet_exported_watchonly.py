...
class WalletExportedWatchOnly(BitXTestFramework):
    ...
    def test_basic_export(self):
        ...
        offline_wallet.getrawchangeaddress()
        for _ in range(KEYPOOL_SIZE * 2):
            assert_equal(online_wallet.getnewaddress(), offline_wallet.getnewaddress())
            assert_equal(online_wallet.getrawchangeaddress(), offline_wallet.getrawchangeaddress())
    ...
    def test_export_with_txs_and_locked_coins(self):
        ...
        # lock some coins
        utxos = offline_wallet.listunspent()
        persistent_lock = [{"txid": utxos[0]["txid"], "vout": utxos[0]["vout"]}]
        temp_lock = [{"txid": utxos[1]["txid"], "vout": utxos[1]["vout"]}]
        ...
        # Export the watchonly wallet file and load onto online node
        # There should not be a rescan as the nodes were at the same height
        with self.online.assert_debug_log(expected_msgs=[], unexpected_msgs=["Rescanning last"]):
        ...
        # Verify the transactions are all output by listtransactions, in the same order
        assert_equal(online_wallet.listtransactions(), offline_wallet.listtransactions())

        # New transactions should be the newest (last) transaction in listtransactions
        new_txid = self.funder.sendtoaddress(online_wallet.getnewaddress(), 1)
        online_wallet.syncwithvalidationinterfacequeue()
        assert_equal(online_wallet.listtransactions()[-1]["txid"], new_txid)
