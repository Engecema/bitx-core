...
class BitXTestMetaClass(type):
    """Metaclass for BitXTestFramework.

    Ensures that any attempt to register a subclass of `BitXTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BitXTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BitXTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BitXTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BitXTestFramework(metaclass=BitXTestMetaClass):
    """Base class for a bitx test script.

    Individual bitx test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave bitxds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bitx-cli instead of RPC for all commands")
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("BitXRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a bitxd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple bitxd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the bitxd
    ...
    def _start_logging(self):
        ...
        # Format logs the same as bitxd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bitx.conf
    ...
    def skip_if_no_bitxd_tracepoints(self):
        """Skip the running test if bitxd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bitxd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitxd_zmq(self):
        """Skip the running test if bitxd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bitxd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bitx-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bitx-wallet has not been compiled")

    def skip_if_no_bitx_tx(self):
        """Skip the running test if bitx-tx has not been compiled."""
        if not self.is_bitx_tx_compiled():
            raise SkipTest("bitx-tx has not been compiled")

    def skip_if_no_bitx_util(self):
        """Skip the running test if bitx-util has not been compiled."""
        if not self.is_bitx_util_compiled():
            raise SkipTest("bitx-util has not been compiled")

    def skip_if_no_bitx_chainstate(self):
        """Skip the running test if bitx-chainstate has not been compiled."""
        if not self.is_bitx_chainstate_compiled():
            raise SkipTest("bitx-chainstate has not been compiled")

    def skip_if_no_bitx_bench(self):
        """Skip the running test if bench_bitx has not been compiled."""
        if not self.is_bench_compiled():
            raise SkipTest("bench_bitx has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bitx-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bitx-cli has not been compiled.")
    ...
    def is_bench_compiled(self):
        """Checks whether bench_bitx was compiled."""
        return self.config.getboolean("components", "BUILD_BENCH")

    def is_cli_compiled(self):
        """Checks whether bitx-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bitx-wallet was compiled."""
        return self.config.getboolean("components", "ENABLE_WALLET_TOOL")

    def is_bitx_tx_compiled(self):
        """Checks whether bitx-tx was compiled."""
        return self.config.getboolean("components", "BUILD_BITX_TX")

    def is_bitx_util_compiled(self):
        """Checks whether bitx-util was compiled."""
        return self.config.getboolean("components", "ENABLE_BITX_UTIL")

    def is_bitx_chainstate_compiled(self):
        """Checks whether bitx-chainstate was compiled."""
        return self.config.getboolean("components", "ENABLE_BITX_CHAINSTATE")
