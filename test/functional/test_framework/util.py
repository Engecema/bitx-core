...
class Binaries:
    """Helper class to provide information about bitx binaries
    ...
    def __init__(self, paths, bin_dir, *, use_valgrind=False):
        ...
        self.valgrind_cmd = [
            ...
            "--trace-children=yes",  # Needed for 'bitx' wrapper
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke bitxd"
        return self._argv("node", self.paths.bitxd, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke bitx-cli"
        # Add -nonamed because "bitx rpc" enables -named by default, but bitx-cli doesn't
        return self._argv("rpc", self.paths.bitxcli) + ["-nonamed"]

    def bench_argv(self):
        "Return argv array that should be used to invoke bench_bitx"
        return self._argv("bench", self.paths.bitx_bench)

    def tx_argv(self):
        "Return argv array that should be used to invoke bitx-tx"
        return self._argv("tx", self.paths.bitxtx)

    def util_argv(self):
        "Return argv array that should be used to invoke bitx-util"
        return self._argv("util", self.paths.bitxutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke bitx-wallet"
        return self._argv("wallet", self.paths.bitxwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke bitx-chainstate"
        return self._argv("chainstate", self.paths.bitxchainstate)

    def _argv(self, command, bin_path, *, need_ipc=False, use_gui=False):
        """Return argv array that should be used to invoke the command.

        It either uses the bitx wrapper executable (if BITX_CMD, need_ipc,
        or use_gui are set), or the direct binary path (bitxd, etc). When
        ...
        (previous releases). Also, valgrind will only apply to the bitx
        wrapper executable directly, not to the commands that `bitx` calls.
        """
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.bitx_cmd is not None or need_ipc or use_gui:
            # If the current test needs IPC or GUI functionality, use the
            # bitx wrapper binary and add appropriate options.
            bitx_cmd = self.paths.bitx_cmd or [self.paths.bitx_bin]
            subcommand = "gui" if use_gui and command == "node" else command
            return self.valgrind_cmd + bitx_cmd + (["-m"] if need_ipc else []) + [subcommand]
        else:
...
def get_binary_paths(config):
    ...
    binaries = {
        "bitx": "BITX_BIN",
        "bitxd": "BITXD",
        "bench_bitx": "BITX_BENCH",
        "bitx-cli": "BITXCLI",
        "bitx-util": "BITXUTIL",
        "bitx-tx": "BITXTX",
        "bitx-chainstate": "BITXCHAINSTATE",
        "bitx-wallet": "BITXWALLET",
    }
    # Set paths to bitx core binaries allowing overrides with environment
    # variables.
    ...
    # BITX_CMD environment variable can be specified to invoke bitx
    # wrapper binary instead of other executables.
    paths.bitx_cmd = shlex.split(os.getenv("BITX_CMD", "")) or None
    return paths
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `BitXTestFramework` or `P2PInterface` class ensures the timeout is
...
def bpf_cflags():
    return [
        ...
        "-Wno-duplicate-decl-specifier",  # https://github.com/bitx/bitx/issues/32322
    ]
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "bitx.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "BitX"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/BitX"
        else:
            datadir = temp_dir / ".bitx"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "bitx.conf"), 'a') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "bitx.conf")):
        with open(os.path.join(datadir, "bitx.conf"), 'r') as f:
            for line in f:
