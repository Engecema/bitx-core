...
class InitTest(BitXTestFramework):
    """
    ...
    def init_stress_test_interrupt(self):
        ...
        def sigterm_node():
            if platform.system() == 'Windows':
                ...
                # bitxd to perform any shutdown logic.
    ...
    def init_pid_test(self):
        BITX_PID_FILENAME_CUSTOM = "my_fancy_bitx_pid_file.foobar"

        self.log.info("Test specifying custom pid file via -pid command line option")
        custom_pidfile_relative = BITX_PID_FILENAME_CUSTOM
        ...
        assert not (datadir / BITX_PID_FILENAME_DEFAULT).exists()
        assert (datadir / custom_pidfile_relative).exists()
        self.stop_node(0)
        assert not (datadir / custom_pidfile_relative).exists()

        custom_pidfile_absolute = Path(self.options.tmpdir) / BITX_PID_FILENAME_CUSTOM
    ...
    def init_rlimit_test(self):
        """Test that bitxd starts correctly when the soft RLIMIT_NOFILE limit is RLIM_INFINITY."""
    ...
    def init_rlimit_large_test(self):
        """Test that bitxd starts correctly when the soft RLIMIT_NOFILE limit is above INT_MAX."""
        if self.RLIM_INFINITY is None:
            self.log.warning("Skipping: resource module not available")
            return
