...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"bitx-cli -h\" for help or \"bitx-cli listwallets\" to see "
...
class TestBitXCli(BitXTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `bitx-cli getblockchaininfo`")
        ...
        # Prevent bitx-cli from using existing rpcport in conf
        ...
        assert_raises_process_error(1, "Cookie file credentials were invalid and no rpcpassword was specified.", self.nodes[0].cli(f"-rpccookiefile={self.nodes[0].datadir_path / 'bitx.conf'}").echo)
        ...
        self.log.info("Test -getinfo and bitx-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and bitx-cli getwalletinfo return expected wallet info")
            ...
            # Test bitx-cli -generate with -rpcwallet in multiwallet mode.
        ...
        if not self.is_ipc_compiled():
            ...
            self.log.info("Test bitx-cli -ipcconnect triggers error if not built with IPC support")
            # node.cli.options includes -rpcconnect which can't be combined with -ipcconnect, so pass just -datadir directly to keep bitx-cli on the test's bitx.conf
            args = self.nodes[0].binaries.valgrind_cmd + [self.nodes[0].binaries.paths.bitxcli, f"-datadir={self.nodes[0].datadir_path}", "-ipcconnect=unix", "-getinfo"]
            result = subprocess.run(args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
            assert_equal(result.stdout, "error: bitx-cli was not built with IPC support\n")
            assert_equal(result.stderr, None)
            assert_equal(result.returncode, 1)


if __name__ == '__main__':
    TestBitXCli(__file__).main()
