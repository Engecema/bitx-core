...
class ChainstateWriteCrashTest(BitXTestFramework):
    ...
    def restart_node(self, node_index, *, expected_tip):
        ...
        while time.time() - time_start < 120 * self.options.timeout_factor:
            try:
                ...
                # If bitxd exits, then try again.  wait_for_node_exit()
                # enforces that bitxd crashed.
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1

        # If we got here, bitxd isn't coming back up on restart.  Could be a
        # bug in bitxd, or we've gotten unlucky with our dbcrash ratio --
        ...
        for i in range(3):
            ...
            for (block_hash, block) in blocks:
                ...
                if not self.submit_block_catch_error(i, block):
                    ...
                    # wait_for_node_exit() enforces that bitxd crashed.
